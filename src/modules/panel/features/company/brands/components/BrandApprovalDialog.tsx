...
interface BrandApprovalDialogProps {
  isOpen: boolean;
  onClose: () => void;
...
export const BrandApprovalDialog: React.FC<BrandApprovalDialogProps> = ({
  isOpen,
  onClose,
  onApprove,
  brandName,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Brand Approval</DialogTitle>
          <DialogDescription>
            {brandName && `Review and approve/reject ${brandName}`}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <DialogFooter className="gap-2">
            ...
            >
              {isLoading
                ? "Processing..."
                : `${status === "approved" ? "Approve" : "Reject"} Brand`}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
