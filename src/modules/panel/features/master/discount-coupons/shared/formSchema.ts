...
export const couponFormInputSchema = z.object({
  ...
  discountValue: z
    ...
    .refine((val) => !isNaN(Number(val)) && Number(val) >= 0, "Discount value must be a positive number"),
  usageLimit: z
    .string()
    .min(1, "Usage limit is required")
    .refine((val) => !isNaN(Number(val)) && Number(val) >= 1, "Usage limit must be at least 1"),
...
export const couponFormSchema = couponFormInputSchema.transform((data) => ({
  ...data,
...
export const couponSchema = {
  basic_information: ({ mode }: ModeProps): FormFieldConfig<CouponFormData>[] => [
    {
      name: "code",
      label: "Coupon Code",
      type: INPUT_TYPES.TEXT,
      placeholder: "e.g., SAVE20, BOGO50",
      description: "Unique code for the coupon (uppercase letters, numbers, hyphens, underscores only)",
      required: true,
