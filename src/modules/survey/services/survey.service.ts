...
import type { PaginationApiResponse } from "@/core/types";
import type {
...
export async function apiCreateSurvey(
  ...
  signal?: AbortSignal,
): Promise<{ statusCode: number; success: boolean; message: string; data: Survey }> {
...
export async function apiGetSurveyStats(
  surveyId: string,
  signal?: AbortSignal,
): Promise<{ statusCode: number; success: boolean; message: string; data: SurveyStats }> {
...
export async function apiEstimateEligibleRespondents(
  ...
  return api.post(`${SURVEY_BASE}/estimate-eligible-respondents`, params, { signal });
}

export async function apiGetMetroCities(
  signal?: AbortSignal,
): Promise<{
  statusCode: number;
  success: boolean;
...
export async function apiInitiateSurveyPayment(
  ...
  return api.post(`${SURVEY_BASE}/${surveyId}/payment/initiate`, {}, { signal });
}
...
export async function apiVerifySurveyPayment(
  ...
  return api.post(`${SURVEY_BASE}/${surveyId}/payment/verify`, data, { signal });
}
